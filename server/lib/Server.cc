...
void Room::enter(std::shared_ptr<Participant> participant,
...
void Room::broadcast(std::array<char, MAX_IP_PACK_SIZE>& message,
  ...
  std::for_each(
      ...
      boost::bind(&Participant::onMessage, _1, std::ref(formattedMessage)));
}
...
void Session::nicknameHandler(const boost::system::error_code& err) {
  ...
  boost::asio::async_read(
      ...
      strand.wrap(boost::bind(&Session::readHandler, shared_from_this(), _1)));
}
...
void Session::readHandler(const boost::system::error_code& err) {
  if (!err) {
    ...
    boost::asio::async_read(
        socket, boost::asio::buffer(readMessage, readMessage.size()),
        strand.wrap(
            boost::bind(&Session::readHandler, shared_from_this(), _1)));
  } else {
...
void Server::onAccept(std::shared_ptr<Session> newSession,
